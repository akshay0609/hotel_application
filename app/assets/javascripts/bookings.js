...
$(document).ready(function(){
  ...
  room_change_or_update = room_selected.length
})
...
function checkbox_click(){
    $("input[type=checkbox]").on('click',function () {
        if (this.checked) {
            room_change_or_update = (room_change_or_update + 1)
        }else{
            room_change_or_update = (room_change_or_update - 1)
        }
        price()
...
function price(){
    ...
    if (room_change_or_update != 0) {
        result = (result * room_change_or_update)
        $('#amount').text(result);
